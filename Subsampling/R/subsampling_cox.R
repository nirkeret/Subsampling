...
get_sampling = function(method, U_coef, V, R, D, X) 
{
  ...
  if(is.null(R))
  {
    fit_samp_opt = survival::coxph(survival::Surv(time=V[samp_ord],event=D[samp_ord]) ~ X[samp_ord,],weights = rand_sampling$weights,robust = F,init = U_coef)
  }else
  {
    fit_samp_opt = survival::coxph(survival::Surv(R[samp_ord],V[samp_ord],D[samp_ord],type = "counting") ~ X[samp_ord,],weights = rand_sampling$weights,robust = F,init = U_coef)
  }
...
#' Subsampling by Cox's method (update with Nir)
#'
#' This function *** (update with Nir).
#' @param V observed times.
#' @param D status (T/F).
#' @param X covariate matrix.
#' @param R recruitment (left truncation) times.
#' @param q0 number of subsampled censored observations for the uniform estimator (being the pilot estimator for L and A). Defaults to be the same as q..
#' @param q number of subsampled censored observations in the second round for the L and A.
#' @param method one of "U"/"L"/"A" (standing for uniform, L-optimal, A-optimal).
#' @return The calculated subsampling.
#' @export
#' @examples
#' subsampling_cox(V, D, X, R, q, q0, "U")
...
subsampling_cox = function(V, D, X, R=NULL, q, q0 = q, method)
{
  ...
  if(is.null(R))
  {
    fit_samp_unif = survival::coxph(survival::Surv(time = V[uni_samp$samp], event = D[uni_samp$samp]) ~ X[uni_samp$samp,],weights = uni_samp$weights[uni_samp$samp],robust = F)  
  }else
  {
    fit_samp_unif = survival::coxph(survival::Surv(R[uni_samp$samp],v[uni_samp$samp],delta[uni_samp$samp],type = "counting") ~ X[uni_samp$samp,],weights = uni_samp$weights[uni_samp$samp],robust = F)  
  }
